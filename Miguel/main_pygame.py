...
while running:
    ...
    if running:
        if modo_jogo == 2 and current_player == 2:
            ...
            if movimento[0][0] == "drop":
                drop(matrix, current_player, movimento[0][1])
            else:
                pop(matrix, current_player, movimento[0][1])

            screen.fill(black)
            draw_board(matrix)
            pygame.display.update()

            if check_victory(matrix, 1) and check_victory(matrix, 2):
                show_end_popup(0)
            elif check_victory(matrix, 1):
                show_end_popup(1)
            elif check_victory(matrix, 2):
                show_end_popup(2)
                running = False
            else:
                current_player = 1
            ...
            # AI vs AI mode with chosen first player
            if ai_vs_ai_choice == "mcts_first":
                # MCTS (Player 1) goes first, Minimax (Player 2) goes second
                if current_player == 1:
                    movimento = MCTS.algoritmo_mcts(matrix, current_player, 5000)
                    if movimento[0][0] == "drop":
                        drop(matrix, current_player, movimento[0][1])
                    ...
                    if check_victory(matrix, current_player):
                        show_end_popup(3)  # MCTS wins
                        running = False
                    else:
                        current_player = 2
                
                elif current_player == 2:
                    movimento = mmp.get_best_move(matrix, current_player, max_depth=10)
                    if movimento is not None:
                        ...
                        if check_victory(matrix, current_player):
                            show_end_popup(4)  # Minimax wins
                            running = False
                        else:
                            current_player = 1
            
            elif ai_vs_ai_choice == "minimax_first":
                # Minimax (Player 1) goes first, MCTS (Player 2) goes second
                if current_player == 1:
                    movimento = mmp.get_best_move(matrix, current_player, max_depth=10)
                    if movimento is not None:
                        if movimento[0] == "drop":
                ...
                import random
                if current_player == 1:
                    # Determine which AI plays as player 1 randomly at start
                    if not hasattr(show_ai_vs_ai_menu, 'first_assigned'):
                        show_ai_vs_ai_menu.first_assigned = True
                        if random.choice([True, False]):
                            # MCTS as player 1, Minimax as player 2
                            show_ai_vs_ai_menu.mcts_is_player1 = True
                        else:
                            # Minimax as player 1, MCTS as player 2
                            show_ai_vs_ai_menu.mcts_is_player1 = False
                    
                    if show_ai_vs_ai_menu.mcts_is_player1:
                        movimento = MCTS.algoritmo_mcts(matrix, current_player, 5000)
                        if movimento[0][0] == "drop":
                            drop(matrix, current_player, movimento[0][1])
                        else:
                    ...
                    winner_display = 3 if show_ai_vs_ai_menu.mcts_is_player1 else 4
                    if check_victory(matrix, current_player):
                        show_end_popup(winner_display)
                        running = False
                    else:
                        current_player = 2
                    ...
                    winner_display = 4 if show_ai_vs_ai_menu.mcts_is_player1 else 3
                    if check_victory(matrix, current_player):
                        show_end_popup(winner_display)
                        running = False
                    else:
